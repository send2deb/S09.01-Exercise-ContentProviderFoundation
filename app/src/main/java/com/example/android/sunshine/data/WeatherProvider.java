...
//  COMPLETED (8) Provide an implementation for the query method
     ...
     * @return retCursor A Cursor containing the results of the query. In our implementation,
     */
    @Override
...
//      COMPLETED (9) Handle queries on both the weather and weather with date URI
        switch (match) {
            case CODE_WEATHER:
                retCursor = sqLiteDatabase.query(WeatherContract.WeatherEntry.TABLE_NAME,
                        projection,
                        selection,
                        selectionArgs,
                        null,
            ...
            case CODE_WEATHER_WITH_DATE:
                String date = uri.getPathSegments().get(1).toString();
                retCursor = sqLiteDatabase.query(WeatherContract.WeatherEntry.TABLE_NAME,
                        projection,
                ...
                break;
            default:
                throw new UnsupportedOperationException("Error in query for uri -> " + uri);
        }
